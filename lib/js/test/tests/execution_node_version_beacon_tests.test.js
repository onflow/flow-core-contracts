...
describe("ExecutionNodeVersionBeacon Contract Tests", () => {
    ...
    test("Change versionUpdateBuffer & versionUpdateBufferVariance succeed", async () => {
        // Contract initialization arguments
        const beginningBuffer = 10;
        const beginningBufferVariance = 0.5;
        // Buffer and variance we'll change and expect after checking
        const expectedBuffer = 12;
        const expectedBufferVariance = 0.75

        // Get account and deploy contract to account
        const ExecNodeVersionBeaconAcct = await getAccountAddress("ExecutionNodeVersionBeaconAddress");
        await deployContract(ExecNodeVersionBeaconAcct, [beginningBuffer, beginningBufferVariance]);
        ...
        // Change versionUpdateBufferVariance
        await sendTransactionByFilenamePasses(
            TRANSACTION_FILENAMES["changeVersionUpdateBufferVarianceFilename"],
            [expectedBufferVariance],
            [ExecNodeVersionBeaconAcct]
        );

        // Check that the versionUpdateBufferVariance has changed as expected
        const actualBufferVariance = await executeScriptByFilename(
            SCRIPT_FILENAMES["getVersionUpdateBufferVarianceFilename"]
        );
        expect(parseFloat(actualBufferVariance)).toEqual(expectedBufferVariance);
    ...
    test("Change versionUpdateBuffer outside of versionUpdateBufferVariance fails", async () => {
        // Contract initialization arguments
        const beginningBuffer = 10;
        const expectedBufferVariance = 0.5;
        // Buffer we'll try to change to
        const newBuffer = 20;

        // Get account and deploy contract to account
        const ExecNodeVersionBeaconAcct = await getAccountAddress("ExecutionNodeVersionBeaconAddress");
        await deployContract(ExecNodeVersionBeaconAcct, [beginningBuffer, expectedBufferVariance]);

        // Change versionUpdateBuffer beyond the what the variance allows
        await sendTransactionByFilenameReverts(
            TRANSACTION_FILENAMES["changeVersionUpdateBufferFilename"],
            [newBuffer],
            [ExecNodeVersionBeaconAcct]
