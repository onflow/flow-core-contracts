...
describe("NodeVersionBeacon Contract Tests", () => {
    ...
    test("Add a version, then emit the table updated event, next sequence should be 1", async () => {
        // Contract initialization arguments
        const expectedBuffer = 10;    
        ...
        const emitEventResult = await shallPass(
            sendTransaction({
        ...
        // Check event emitted
        expect(emitEventResult[0].events[0].data.versionTable).toEqual(expectedVersionTable);
        expect(parseInt(updatedTableNextSequenceNumber)).toEqual(1);
    });


    test("emitNodeVersionTableUpdated emits event with changes since last call", async () => {
        // Contract initialization arguments
        const expectedBuffer = 10;
        ...
        // no changes were made to the table, so no event
        ...
        expect(emitEventResult[0].events).empty()
        ...
        expect(emitEventResult[0].events.size).toEqual(1)
        ...
        expect(emitEventResult[0].events.size).empty()
    });
